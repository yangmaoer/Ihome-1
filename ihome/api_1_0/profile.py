...
#     name = req_data.form.get('name')
#     print(name)
#     if not name:
#         return jsonify(errno=RET.PARAMERR, errmsg='username error')
#
#     # 保存数据库
#     try:
...
#         db.session.rollback()
#         current_app.logger.error(e)
#         return jsonify(errno=RET.DBERR, errmsg='名字保存数据库错误')
#     # 保存成功返回
...
# 获取用户信息的后端代码
@api.route('/user/center', methods=['GET'])
@login_required
